...
class StrictValidator {
  ...
  static validateHooks(loaded, report) {
    ...
    // Hook logic now lives entirely in plugkit (Rust). hooks/ ships only
    // hooks.json describing which plugkit subcommands fire on which events.
    if (hookNames.length === 0) {
      const fs = require('fs');
      const path = require('path');
      const hooksJson = path.join(loaded.hooksDir || '', 'hooks.json');
      if (fs.existsSync(hooksJson)) {
        report.info.push('✓ hooks/hooks.json present (plugkit handles hook events)');
        return;
      }
      report.errors.push('CRITICAL: No hooks/hooks.json found. plugkit needs a manifest declaring which events it handles.');
      return;
    }
  ...
  static validateFileIntegrity(loaded, report) {
    ...
    if (fs.existsSync(loaded.hooksDir)) {
      const hookFiles = fs.readdirSync(loaded.hooksDir);
      if (hookFiles.length === 0) {
        report.errors.push('CRITICAL: hooks/ directory exists but is empty (need at least hooks.json)');
      }
    }
