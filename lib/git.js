...
async function push(branch = 'main', sessionId = 'unknown') {
  ...
  const pre = await status(sessionId);
  if (pre.isDirty) {
    emitGitEvent('error', 'push refused: worktree dirty', { branch, sessionId, modified: pre.modified.length, untracked: pre.untracked.length });
    throw new Error(`Push refused: worktree dirty (modified=${pre.modified.length} untracked=${pre.untracked.length} deleted=${pre.deleted.length}). Commit or revert before push — a push from a dirty tree orphans the unstaged delta.`);
  }
