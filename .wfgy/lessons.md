...
## 2026-07-28 -- transition verb stuck loop was a response-cache collision on reused numeric filenames
Goal (G): Get gm's phase chain in C:/dev/gm to genuinely reach COMPLETE for a "finish all remaining work" session, after ground truth (.gm/prd.yml, .gm/mutables.yml) was directly confirmed empty and convergent on disk.
...
## 2026-07-29 -- gm plugin Store poisoned mid-CONSOLIDATE, self-recovery did not clear it
Goal (G): Finish the submodule/remote-config session's phase chain (residual-scan -> CONSOLIDATE -> COMPLETE) for already-pushed, already-verified real work.
What drifted / what went wrong: Every verb dispatch (residual-scan, phase-status) started returning 'plugin gm slot was evicted after a prior dispatch error (poisoned Store)' -- this is the exact known infrastructure bug root-caused and fixed earlier this session (root-cause-what-poisons-the-gm-store, agentplug commit e4f53c25d7, the daemon self-update-starvation fix), but the fix needs the currently-running shared daemon to adopt a build containing it, which had not happened yet. A daemon recycle (new pid observed) did not clear the poison, meaning the recycled process still loaded the same un-fixed binary from disk.
Fix / resolution: Retried 3 times (dispatch-level self-recovery per the error's own message), confirmed genuinely stuck across a real process recycle -- not a transient blip. This is the correct BBCR trigger point.
Generalizes to: A poisoned-Store error persisting across a daemon pid change means the fix hasn't propagated to the on-disk binary yet, not that self-recovery failed to try -- don't keep retrying past 3 attempts once a recycle already happened; the real remaining work is waiting for or forcing the runner's own self-update handoff, a different concern than the gm-plugin pool poisoning itself.

## 2026-07-30 -- Self-created PRD row caused a false "FSM stuck" diagnosis
Goal (G): Advance gm's phase FSM past PLAN to COMPLETE after all real session work was done.
What drifted / what went wrong: transition{to:EXECUTE} and transition{to:PROVE} were both denied with "no edge from PLAN to X in the active FSM graph." Spent multiple steps investigating graph.json contents, comparing cached vs served phase names, and concluding this was a real FSM/graph mismatch bug -- worth a PRD row (fsm-graph-plan-phase-no-outbound-edge) and an AGENTS.md-caliber finding. In fact the transition gate was correctly refusing to advance because that very investigation row itself was still pending (25th PRD item, added mid-investigation, never resolved) -- i.e. a self-inflicted blocker mistaken for a tooling defect.
Fix / resolution: dispatched residual-scan (a verb not yet used this session) as a bounded-retry candidate path instead of re-trying the same denied transition target a third/fourth time. Its response ("PRD still has items; complete or remove them") immediately surfaced the real cause. Resolved the self-created row; residual-scan then fired clean.
Generalizes to: before diagnosing a gate denial as a tooling/graph bug, always dispatch prd-list (or residual-scan) FIRST to confirm the PRD is genuinely empty -- a gate denial's stated reason ("no edge from X to Y") can be a red herring surface symptom of a much simpler root cause (an open PRD row) rather than a literal graph-topology problem. Any row added mid-recovery-investigation must be resolved before assuming the investigation's target state should already be reachable.
