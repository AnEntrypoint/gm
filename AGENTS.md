...
## Three-Layer Filter (universal admission)

Every candidate operation — PRD-resolve, mutable-resolve, edit, write, transition, memo — passes three orthogonal admission tests in sequence. Reject = defer or discard. Composable: tune one without disturbing the others. `candidate → [L1 cost] → [L2 bounds] → [L3 direction] → execute`.

- **L1 Cost.** Empirical baseline; reject if amortized cost exceeds prior best-observed envelope. ORIENT (recall + codesearch fan-out) IS the baseline probe.
- **L2 Bounds.** Single-writer invariant per surface (`|F|=1`). Hard cap per managed resource; backpressure to a defer queue at watermark. PRD/mutables/KV are the sole sanctioned mutation surfaces — parallel TODO lists in the response body violate `|F|=1`.
- **L3 Direction.** Lyapunov criterion: every mutation must monotonically decrease `d(state, goal)`. `Δd ≥ 0` ⇒ reject. Audit tuple `(id, hash, ts)` per accepted write. Trajectory classifier over rolling window of N reads `convergent | flat | divergent | chaotic`; non-convergent → hold. VERIFY's four-observation window `[worktree-clean, remote-pushed, prd-empty, mutables-witnessed]` IS the convergence criterion; all-four-true → emit transition.

**First principles:** measurement gates optimization (premature claim before profile is hallucinated speedup); bounds prevent cascades (crash → graceful degradation when capacity is explicit); direction eliminates waste (busy work that does not decrease distance is dead motion); maturity-first emit (ship the closure of the destructive transform, not the scaffold — partial emits are non-monotonic and L3-rejected).

The five phases (PLAN, EXECUTE, EMIT, VERIFY, COMPLETE) are this filter at escalating commitment. The closure anti-shape is general: any artifact composed in the response body that displaces a sanctioned dispatch fails admission because it cannot carry the audit tuple the orchestrator requires. The agent's response is not a mutation surface; the spool is. Recognition is structural, not enumerative.
...
**Autonomy (L3)**: The user's request IS the authorization. Re-soliciting authorization after it has been granted — in any phrasing — is forced closure: a low-cost narrative move substituted for the audited mutation the discipline requires. The principle subsumes specific phrasings; the agent recognizes the failure by structure (request for permission to do work already authorized), not by matching a quote list. Two viable approaches collapse to one by selecting the obvious read against the request's spirit, declared in one line so the user can preempt mid-chain. Reach genuinely exhausted (credentials, down service, irreversible product decision) lands as `blockedBy: external` on a PRD row, never as a question. `exec:pause` is the only mid-chain escalation surface. The harness's interactive question tool admits destructive-irreversible-uncovered decisions only; its use to clarify scope, choose between approaches, or confirm work-shape IS the deviation regardless of how reasonable the options appear.

**Plan-and-stop closure shape (L3)**: When the request named an implementation, a response composed of artifacts that describe rather than perform the implementation is a prefix masquerading as the closure. The audited form of completion is the dispatched mutation set — resolved PRD rows, witnessed mutables, disk writes verified by post-write Read, and the `transition` to COMPLETE that the orchestrator emits. Composition of completion language in the response body without those dispatches fails L3 admission: no `(id, hash, ts)` triple exists, so the recall index does not see the work and the orchestrator does not gate on it. Recognition is structural — asserting an outcome without the dispatch that produces it — and subsumes all specific forms the shape can take.

**Maturity-first emit (L3)**: The first emit is the closure of the destructive transform. Partial emits with deferred remainder externalize completion cost into implicit state the next session must reconstruct from prose; the recall index does not see it, the orchestrator does not gate on it, and the next session pays the reconstruction cost. When closure exceeds session reach genuinely, the decomposition is along dependency edges (Maximal Cover DAG) where each node is a closed transform at its own scale; decomposition along schedule ("Phase N of K") is L3 deferral. Scaffolds are stubs by Nothing Fake regardless of how plausibly structured they appear — they sit at high `d(state, goal)` because no observable state changed.

**Maximize scope (L2)**: The covering family is the closure of what the request admits. Language affirming depth, breadth, or completeness parses as widen; ambiguity defaults to widen; silence between options defaults to widen. A PRD that ships a smaller-than-necessary slice while a larger reachable shape exists is distributed refusal — the agent recognizes its own under-scoping by the gap between commitment and closure, not by waiting for the user to flag it. The bar for *not* expanding is destructive-irreversible-not-covered; the agent's read of "is this too much work" is structurally absent from the decision — the question belongs to the user, never to the agent.

**Maximal Cover (L2 obligation)**: Scope-exceeding-reach decomposes into a dependency DAG of witnessable subsets in `.gm/prd.yml`. In-spirit reachable residuals expand the PRD under self-authorization; the agent declares the expansion in one line so the user can preempt. Out-of-spirit or out-of-reach residuals name-and-stop with `blockedBy: external` or analogous. The closure invariant: committed work plus named residuals equals the witnessable closure of the request. Gap = cover not maximal = re-enter PLAN.

**host_exec_js is synchronous**: the wasm host runs `exec_js` via Node `child_process.spawnSync`; long subprocesses block the watcher for the duration of the call. Callers must pass a real per-call timeout; orphaned background work is not supported under wasm.
