...
- **Plain-text-body verbs**: `exec_js` (and every language stem it backs -- `bash`/`python`/`powershell`/etc) and `serp`/`browser`/`cdp` are the exception to every other verb's structured-JSON body -- their body is the raw code/script text itself, never `{"code": "..."}` or `{"body": "..."}`. A JSON-object body is rejected up front with a clear error naming the supported shapes; this rejection is deliberate, not a bug to route around by re-wrapping. Internally, these verbs' small metadata (`timeoutMs`, `engine`, ...) rides a separate host-import parameter alongside the raw body, never JSON-wrapped around it -- so the raw text is never JSON-escaped/re-parsed on its way across the wasm boundary.
...
Push to any rs-* sibling -> `cascade.yml` -> rs-plugkit `release.yml` -> single `plugkit.wasm` (npm `plugkit-wasm` + `plugkit-bin` Releases) -> best-effort gm-metadata-sync commit -> `publish.yml` uploads a `gm-skill-<version>.tar.gz` GitHub Release (skills+SKILL.md mirror; no npm registry for gm-skill/gm-plugkit). Step sequence + PUBLISHER_TOKEN: the recall store (`recall: cascade pipeline`).
