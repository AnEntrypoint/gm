...
2026-04-28: CONTEXT TO MEMORIZE facts audit. All 5 source facts (rs-search Windows nightly, PDF ingestion, gitignore re-include, rs-learn DB path, plugkit runtime-fetch) verified present in AGENTS.md, non-changelog-shaped, justified. Recall audit: tested 5 stable items (PLATFORM_META, cleanBuildDir, conhost flag, rs-search nightly, PDF cache) — 0/5 accurate+complete match. Refined 3 high-load items via memorize. All 5 context facts retained; store improving.
