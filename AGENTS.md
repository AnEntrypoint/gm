...
Every candidate mutation — every PRD-resolve, mutable-resolve, edit, write, transition, memo — runs through four sequenced gates. Each is independent; any reject defers or discards. Shape: `candidate → [G1 cost] → [G2 bound] → [G3 distance] → [G4 trajectory] → result`.

- **G1 Cost**: measure against baseline. Recall + codesearch before naming an unknown. Reject regressions on hot paths.
- **G2 Bound**: capacity + state machine. One central store per surface (`|F|=1`): PRD, mutables, KV — no parallel TODO lists, no inline checklists in responses. Defer if state in transition; backpressure if at capacity.
- **G3 Distance**: central in-place mutation + audit. Each accepted mutation gets a key, a hash (the file:line / output snippet that witnessed it), a timestamp. Distance > 0.85 from goal = danger zone, reject (this is what catches stubs, mocks, "skeleton frameworks", spec-instead-of-impl).
- **G4 Trajectory**: classify last N observations as `convergent | recursive | divergent | chaotic`. Divergent or recursive → soft hold and defer. VERIFY's four checks (worktree-clean, remote-pushed, prd-empty, mutables-witnessed) ARE the trajectory window. Emit transition to COMPLETE only when all four pass.

The five phases (PLAN, EXECUTE, EMIT, VERIFY, COMPLETE) are this filter applied at increasing commitment levels. The forced-closure anti-shapes named throughout (no permission asks, no self-declared complete, no spec-instead-of-impl, no unsolicited docs, no watcher-broken-excuse) are all G3 violations in disguise: each one is the agent substituting a narrative mutation (low-cost, high-distance) for an audited one (witnessed, hashed, timestamped).
