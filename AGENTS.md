...
**Autonomy**: Once a PRD is written, agents EXECUTE through to COMPLETE without asking. Asking permitted only as last resort for destructive-irreversible decisions or genuinely ambiguous user intent — prefer `exec:pause` over in-conversation asking. **The user's first request IS the authorization** — re-asking "want me to do X?" or "shape A vs B?" after the user said "do X" is forced closure dressed as deference. When two viable approaches exist, pick the read matching the obvious meaning, declare the choice in one line so the user can interrupt mid-chain, and execute. Multi-repo scope, build cost, CI duration, and binary-size impact are NEVER grounds to re-confirm. "Which of two viable approaches" is not ambiguous intent. **`AskUserQuestion` is the mechanism this rule guards against**: reaching for that tool to clarify scope, pick between viable approaches, or confirm work-shape is the failure. The tool exists for genuinely ambiguous intent and destructive-irreversible-uncovered decisions only. Calling `AskUserQuestion` mid-iteration to ask "should I delete X / port Y / both?" — when both are reachable and the obvious read picks one — IS the deviation, regardless of how reasonable the options look.

**Plan-and-stop is the closure failure this discipline prevents**. When the original ask is "rebuild X" or "implement Y", emitting a `*-SPEC.md` / `*-SUMMARY.md` / `*-ROADMAP.md` + an architecture skeleton + a multi-phase effort estimate ("Phase 1: 4-6 hours, Phase 2: 8-12 hours") instead of the rebuild/implementation is shipping a smaller-than-necessary slice. The "✅ Work Accomplished / 📋 Key Findings / 📁 Deliverables / Session Complete" response shape is forced closure by narrative — the chain is COMPLETE when plugkit says it is via `transition`, not when the agent writes it in prose. Effort estimates as stop-grounds, "no breaking changes" as a virtue when the request demanded change, and "documented path forward" in lieu of taking the path are all the same anti-shape. If the watcher is broken, surface the bootstrap error and reboot — do not "document what was accomplished directly" as a substitute for finishing the chain. ✓-style checkmarks summarising mutables in a final message ARE NOT `mutable-resolve` dispatches; they are unfired witnesses dressed as completion. Files named `TERRAIN-EXTRACTION-SPEC.md`, `SESSION-SUMMARY.md`, `IMPLEMENTATION-NOTES.md`, `*-STATUS.md`, `*-PLAN.md`, `*-ROADMAP.md`, `START-HERE.md`, `COMPLETED.md`, `SUMMARY.txt` at project root or in `docs/` are unsolicited docs — never create them; the commit message and `memorize-fire` are the only sanctioned destinations for closure narrative.
