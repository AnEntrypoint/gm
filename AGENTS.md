...
- **Orchestrator verbs**: `instruction`, `transition`, `transition-revert`, `discipline-check-removal`, `discipline-audit`, `memory-namespace-audit`, `phase-status`, `mutable-resolve`, `mutable-add`, `mutable-list`, `memorize-fire`, `memorize-continue`, `discipline-note`, `residual-scan`, `auto-recall`, `prd-add`, `prd-resolve`, `prd-list`, `task-spawn`, `task-list`, `task-stop`, `task-output`, `fsm-vendor`, `fsm-validate`, `fsm-propose-override`, `claim-audit`, `submodule-check`. `transition-revert` pops one entry off the phase's own LIFO transition-history accumulator (`TurnState.phase_history`), restoring the phase it recorded -- a revertible-effect pairing (each `transition` call composes its inverse onto the accumulator) applied to the phase graph itself, so a feedback-edge re-entry undoes precisely rather than only via mutables.yml/prd.yml side state. `discipline-check-removal` names every enabled discipline still relying on a `provides` key before it is disabled -- the withdrawal-ordering guard (Section "Disciplines are isolated knowledge stores" below has the full field shape). `discipline-audit` runs live checks for preservation/recovery-exactness/ordering/progress plus a fail-closed dangling-`requires` scan. `memory-namespace-audit` runs the same fiber-lifecycle machinery over gm's memory namespaces (a third component kind sharing `fiber_lifecycle.rs` with disciplines and plugins).
...
**Disciplines are isolated knowledge stores**, tracked, `@<name>` sigil-scoped, enabled list at `.gm/disciplines/enabled.txt` (one name per line; `discipline_note.rs::active_policies()` auto-surfaces each enabled discipline's `.gm/disciplines/<name>/policy.md` on every `instruction`). An optional `.gm/disciplines/<name>/requires.json` (`{"requires":["capability-key",...],"provides":["capability-key",...]}`) is that discipline's full coeffect interface -- `requires` is its specification (capability keys it needs), `provides` is what it supplies (defaulting to its own name when omitted, so a bare-name `requires` entry still resolves). `active_policies()` surfaces the discipline only when every required key is provided by some enabled discipline, so an unmet requirement reactively excludes just that discipline rather than blocking the enabled list. Each discipline carries a persisted lifecycle in `.gm/disciplines/<name>/fiber-state.json` (gitignored, `Inactive`/`Active`/`Unloading`), advanced one transition per `active_policies()` call -- `Active -> Unloading` the instant satisfaction is lost, `Unloading -> Inactive` on the following call, giving dependents a real one-dispatch window where a leaving provider is still visible. Dispatch `discipline-check-removal` (`{"discipline":name}`) before disabling a discipline: it names every other enabled discipline whose fiber has reached `Active` and whose `requires` still resolves to the target's `provides` -- the withdrawal-ordering guard (Cordis paper Theorem 63), never disable a discipline with a named dependent still relying on it. `discipline-audit` checks preservation (no two `Active` disciplines share a `provides` capability, enforced by construction via `ActiveFiberSet`'s type-safe insert), recovery exactness and ordering (each also enforced via a proof-carrying type -- `fiber_lifecycle::WithdrawalComplete`/`SafeToWithdraw` -- not merely a runtime check), progress, and fail-closed access control (a `requires` entry naming a capability no known discipline anywhere provides is a dangling reference). A `requires.json` may also set `"realm":"<name>"` (Cordis paper Section 3.2.3, coeffect isolation): two disciplines in different realms providing the same bare capability name do not satisfy each other's `requires` and never collide for preservation -- real multi-tenant isolation, no `realm` field means the default realm. `fiber_lifecycle.rs` is a kind-agnostic module (disciplines, sibling wasm plugins, and gm's own memory namespaces via `memory_component.rs`/`memory-namespace-audit` all instantiate it with zero changes to the module itself). Detail: the recall store (`recall: gm disciplines mechanics`).
...
Detection classes: exact-match known-incident strings (campaign tag, hardcoded C2-lookup address, custom payload-smuggling header), structural patterns (blockchain-RPC-derived C2 address resolution, IP-from-bytes decode, detached+hidden-stdio child-process spawn, `eval()` fed by a network response, XOR-decode loops, `require`/`module` stashed onto `global`, a dense minified tail line appended to an otherwise normal readable file, `\uXXXX`-escaped ASCII used to defeat plaintext grep, multiple public RPC endpoints raced with `Promise.any` for C2 resilience), Unicode confusables/invisible characters (zero-width spaces/joiners, bidi override characters — the Trojan Source class — soft hyphens, and common Cyrillic homoglyphs for Latin letters), and a statistical AI-prose-tell class over `.md` files (sentence-length burstiness, "it's not X, it's Y" and "X, Y, and Z" rhetorical constructs, an overused-vocabulary regex, explicit validation markers) yielding a heuristic `ai_probability_score` — a signal for human-review-worthy docs, not a malicious-code finding.
...
**Every downloaded wasm/binary artifact is sha256-verified against the sha published alongside that exact resolved release, fetched fresh, never against a locally-committed value alone** -- `install.sh`/`install.ps1`. Floating past the committed pin never weakens verification. Detail: the recall store (`recall: gm cascade pin-toil design`).
